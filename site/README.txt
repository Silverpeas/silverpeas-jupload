...
[MAJOR] since 1.7.0 u51 version :
- The certificate used to sign the applet MAY NO MORE be auto-signed. You must do one of these :
   * Make your certificate be signed by a valid Certication Authority,
   * Create your own CA, deploy it on all navigator which will use the applet, and sign with this CA the certificate you'll sign the applet with.  
- The applet permission must be indicated in the manifest.mf file (automatically done in the build, by this version) 


Fixes:
- [Major] Since Java 1.7.0 u51, the manifest file must contain the all-permission tag
- [Doc] The howto sign the applet page has been updated, to indicate how to manage one's own Certificate for Authority (to be improved)
- [Major-dev] Maven build now works also on Linux (Unit tests would fail while building under Linux)
- [Major] Bug 73 - Chunked upload : last chunk could be a little bigger (up to ~1000 bytes?) than expected
- [Minor] Better cleaning of resources, when an error occurs in UploadPolicy.afterUpload (see https://sourceforge.net/projects/jupload/forums/forum/199106/topic/5590495)
- [Minor] mvn site would on maven-translation-plugin would not work with maven 3 
- [Minor] Bug 77 - (jupload.php) fix upload type 'array' when nbFilesPerRequest > 1

Translation:
- New translation (Latvian), thanks to Sergey Nosirev
