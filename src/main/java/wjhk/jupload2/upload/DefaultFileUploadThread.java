...
// Last modified: $Date: 2012-11-11 22:04:26 +0100 (dim., 11 nov. 2012) $
//
...
public abstract class DefaultFileUploadThread extends Thread implements
        ...
        while (!this.fileUploadManagerThread.isUploadFinished()) {
            ...
            try {
                ...
                try {
                    ...
                    // We ignore the error
                    this.uploadPolicy.displayWarn("[run(), After cleanAll] Ignoring "
                            + e.getClass().getName()
                            + " because upload is finished");
                }
            }
     ...
     *             detected.
     */
    void doUpload(UploadFilePacket packet)
        ...
        for (UploadFileData uploadFileData : packet) {
            ...
            this.uploadPolicy.displayDebug(
                    "file " + uploadFileData.getFileName() + ": content="
     ...
     *             detected.
     */
    void doChunkedUpload(UploadFilePacket packet)
            throws JUploadException, JUploadInterrupted {
     ...
     *             detected.
     */
    void doNonChunkedUpload(UploadFilePacket packet,
        ...
        try {
            ...
            for (UploadFileData uploadFileData : packet) {
                ...
                // Actual upload of the file:
                if (!this.fileUploadManagerThread.isUploadFinished()) {
                    uploadFileData.uploadFile(getOutputStream(),
                            uploadFileData.getUploadLength());
                }
